...
CARGO_RELEASES_URL = "https://github.com/rust-lang/cargo/releases/download/v{version}/{asset}"
...
class CargoRelease:
    ...
    async def stable_versions(cls) -> dict:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            gh = GitHubAPI(session, "cargo-version-fetcher", **kwargs)
            version_content = await gh.getitem(
                "/repos/rust-lang/cargo/contents/Cargo.toml"
            )

            cargo_toml = base64.b64decode(version_content["content"]).decode("utf-8")
            
            # Extract version from Cargo.toml
            version_line = next((line for line in cargo_toml.splitlines() if line.strip().startswith("version =")), None)
            if version_line:
                current_version_str = version_line.split("=")[1].strip().strip('"\'')
                current_dev_version = parse(current_version_str.replace("-dev", ""))
                current_minor = current_dev_version.minor
                supported_minors = [current_minor, current_minor - 1, current_minor - 2]
                version_info = collections.defaultdict(list)

                async for release in gh.getiter("/repos/rust-lang/cargo/releases?per_page=100"):
                    ...
                    try:
                        ...
                        if version.major == current_dev_version.major and version.minor in supported_minors:
                            minor_key = f"{current_dev_version.major}.{version.minor}"
                ...
                for minor in sorted(supported_minors, reverse=True):
                    minor_key = f"{current_dev_version.major}.{minor}"
    ...
    async def download(self, binary: str, hash: str) -> pathlib.Path:
        binary_path = pathlib.Path("/tmp/cargo-bins") / self.version / binary
        url = CARGO_RELEASES_URL.format(asset=binary, version=self.version)
    ...
    async def checksums(self) -> dict:
        url = CARGO_RELEASES_URL.format(asset="checksums.txt.asc", version=self.version)
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
