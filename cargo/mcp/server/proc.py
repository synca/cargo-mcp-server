...
from cargo.mcp.server.errors import ProcessError


class ProcessManager:
    def __init__(self, registry_dir="/tmp/cargo_registry"):
    ...
    async def start_cargo(self, binary, config_content, proc_id=None):
        if proc_id is None:
    ...
    async def find_orphans(self):
        ...
        for line in await self.running_processes():
            if "cargo" in line and "-c" in line and str(self.registry_dir) in line:
        ...
        orphans = [p for p in cargo_procs if p["pid"] not in managed_pids]
        return orphans
